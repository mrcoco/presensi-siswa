$(document).ready(function(){
    var url_path = "/tahunajaran/";
    var tahunajaran_grid = $("#grid-tahunajaran").bootgrid({
        ajax: true,
        url: url_path+"list",
        selection: true,
        multiSelect: true,
    ...
    }).on("loaded.rs.jquery.bootgrid", function()
    {
        ...
        $(this).find(".command-edit").on("click", function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    $("#grid-tahunajaran").bootgrid("reload");
                    setTimeout(function(){
                        $('#myModal').modal('hide')
                    }, 10000);
                }
        ...
        $("#create").on("click",function(e)
        {
            ...
            $("#myForm").ajaxForm({
                ...
                success: function(data) {
                    myAlert(data);
                    tahunajaran_grid.bootgrid("reload");
                    setTimeout(function(){
                        $('#mytahunajaran').modal('hide');
                    }, 10000);
                }
            });
        });
    ...
    function myAlert(e)
    {
        ...
        mesg["alert"] = e.alert;
        mesg["title"] = e.msg;
        mesg["msg"] = "#tahunajaran "+e._id+" "+e.msg;
        notif_show(mesg);
    }
