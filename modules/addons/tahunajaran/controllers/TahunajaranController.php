...
class TahunajaranController extends ControllerBase
{
    public function initialize()
    {
        $this->assets
            ->collection('footer')
            ->setTargetPath("themes/admin/assets/js/combined-tahunajaran.js")
            ->setTargetUri("themes/admin/assets/js/combined-tahunajaran.js")
            ->join(true)
            ->addJs($this->config->application->addonsDir."tahunajaran/views/js/js.js")
    ...
    public function listAction()
    {
        ...
        if ($searchPhrase != '') {
            $arProp['conditions'] = "tahun LIKE ?1";
            $arProp['bind'] = array(
                1 => "%".$searchPhrase."%"
            );
        }
        $qryTotal = Tahunajaran::find($arProp);
    ...
    {
        $this->view->disable();
        $data = Tahunajaran::findFirst($this->request->getPost('hidden_id'));
         $data->tahun = $this->request->getPost('tahun');
	 $data->status = $this->request->getPost('status');
    ...
    {
        $data = Tahunajaran::findFirst($this->request->getQuery('id'));
        $response = new \Phalcon\Http\Response();
        $response->setContentType('application/json', 'UTF-8');
    ...
    {
        $this->view->disable();
        $data   = Tahunajaran::findFirstById($id);
